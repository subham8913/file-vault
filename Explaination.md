...
### 💡 Interview Discussion: Alternative "Better" Design

You might be asked: *"Is there a cleaner way to model this?"*

**Your Suggestion (Normalization):**

Yes! A more robust design would separate **Storage** from **Metadata**.

1. **Table `PhysicalFile`:** Stores `hash`, `file_path`, `reference_count`. (No user info).

2. **Table `UserFile`:** Stores `user_id`, `filename`, `upload_date`, and a Foreign Key to `PhysicalFile`.



**Comparison:**

* **Current Design (Self-Referencing):** Simpler to implement (1 table). Good for small projects.

* **Proposed Design (Normalized):** Cleaner separation of concerns.

* If User A deletes their file, we just delete the `UserFile` row.

* The `PhysicalFile` row stays until `count == 0`.

* No "Ghost" records needed in the user's view.

* * **Trade-off:** Requires a JOIN query to get the file path every time.



### 🚀 Production Readiness: "Should I refactor this?"
...
- **Logic:** `backend/files/utils/storage.py` -> `check_storage_quota` (called before upload) and `update_storage_usage` (called after upload/delete).
...
- **Cache-Based:** Uses Django's cache (Redis or memory) to count requests. It uses a "Fixed Window" or "Token Bucket" approach keyed by `user_id`.
...
- **Application:** `backend/files/views.py` -> Look for `@method_decorator(rate_limit(...))` above the `FileViewSet`.
...
Router -->|Delete| DelMeta[Delete User Metadata (File)]
...
- Root directory: `Dockerfile` and `docker-compose.yml`.
